...
def main() -> None:
    ...
    L.append("echo.")
    L.append("")
    L.append("So old and new totals measure different things. Rescoring the *same weights* under the")
    L.append("current scorer:")
    L.append("")
    L.append("| model | old formula | current scorer | |")
    L.append("|---|---:|---:|---|")
    for k in ("base", "sft2"):
        delta = r[k]["mean"] - old[k]
        L.append(f"| {label[k]} | `{old[k]:.4f}` | `{r[k]['mean']:.4f}` | `{delta:+.4f}` |")
    L.append("")
    L.append("The base row is the point: the old formula paid `0.3040` to a model that emits")
    L.append("`**Summary**` instead of `## Summary`, invents sections outside the schema, fabricates")
    L.append("its content, then loops the same two paragraphs until it hits the token cap. The")
    L.append("current scorer pays it `0.0000`.")
    L.append("")
    L.append("### Decoding")
    L.append("")
